...
import { getPaymentReceiptTemplateById, deletePaymentReceiptTemplate } from '../database';
...
type PaymentReceiptTemplateDetailsRouteProp = RouteProp<RootStackParamList, 'PaymentReceiptTemplateDetails'>;
type PaymentReceiptTemplateDetailsNavigationProp = StackNavigationProp<RootStackParamList>;

const PaymentReceiptTemplateDetailsScreen: React.FC = () => {
  const [template, setTemplate] = useState<PaymentReceiptTemplate | null>(null);
  const [loading, setLoading] = useState(true);
  const [menuVisible, setMenuVisible] = useState(false);
  const [exportMenuVisible, setExportMenuVisible] = useState(false);
  const theme = useTheme();
  const route = useRoute<PaymentReceiptTemplateDetailsRouteProp>();
  const navigation = useNavigation<PaymentReceiptTemplateDetailsNavigationProp>();
  ...
  const loadTemplate = async () => {
    try {
      setLoading(true);
      const data = await getPaymentReceiptTemplateById(templateId);
      setTemplate(data);
    } catch (error) {
  ...
  const handleDelete = () => {
    ...
    if (template) {
      confirmDelete(
        ...
        async () => {
          try {
            await deletePaymentReceiptTemplate(template.id!);
            navigation.goBack();
          } catch (error) {
    ...
    if (template) {
      try {
        await generateAndShareTemplateDocument(
          template.id!,
          'payment-receipt', // Changed template type
          undefined,
          undefined,
          undefined,
          companyId
...
export default PaymentReceiptTemplateDetailsScreen;
