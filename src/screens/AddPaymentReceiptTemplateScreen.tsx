...
import { addPaymentReceiptTemplate } from '../database';
import { spacing, shadows } from '../constants/theme';

type AddPaymentReceiptTemplateNavigationProp = StackNavigationProp<RootStackParamList>;

const AddPaymentReceiptTemplateScreen: React.FC = () => {
  const [name, setName] = useState('');
  const [content, setContent] = useState('');
  const [submitting, setSubmitting] = useState(false);
  const theme = useTheme();
  const navigation = useNavigation<AddPaymentReceiptTemplateNavigationProp>();
  ...
  const handleSubmit = async () => {
    ...
    try {
      setSubmitting(true);
      await addPaymentReceiptTemplate({
        name: name.trim(),
...
export default AddPaymentReceiptTemplateScreen;
